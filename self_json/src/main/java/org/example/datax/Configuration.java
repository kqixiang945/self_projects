...
public class Configuration {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        Configuration config = from(new File("/Users/kongxiaohan/project_code/idea/summerchill/bi_task/datax/etl0/dx_e0_bimplus_quality_acceptance_standard.json"));

        //从包括json的InputStream对象加载Configuration
        from(new FileInputStream(new File("/Users/kongxiaohan/project_code/idea/summerchill/bi_task/datax/etl0/dx_e0_bimplus_quality_acceptance_standard.json")));

        //从Map对象加载Configuration
        from(new HashMap<String, Object>());

        //根据用户提供的jsonpath，寻址String对象
        System.out.println(config.getString("job.setting"));

        //根据用户提供的json path，寻址String对象，如果对象不存在，返回默认字符串
        System.out.println(config.getString(("job.setting2"), "这个是默认值1...."));

        //⭐️⭐️⭐️获取到key的值返回 如果是数组类型需要加上[] 如果对应的key没有报错.
        System.out.println(config.getNecessaryValue("job.content[0].reader.parameter.connection[0].querySql[0]",CommonErrorCode.CONFIG_ERROR));

        //⭐️⭐️⭐️获取非必须的key
        System.out.println(config.getUnnecessaryValue("jobb","这个是默认值....", CommonErrorCode.CONFIG_ERROR));

        //用户指定部分path，获取Configuration的子集
        config.getConfiguration("job.content[0].reader.parameter.connection");

        //根据用户提供的json path，寻址Character对象  Character对象，如果path不存在或者Character不存在，返回null  只返回value的第一个元素
        System.out.println(config.getChar("job.content[0].reader.parameter"));
